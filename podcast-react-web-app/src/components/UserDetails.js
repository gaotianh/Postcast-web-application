...
const UserDetails = (user) => {
    ...
    const getAllReviewInfo = async () => {
        ...
        await Promise.all(user.user.reviews.map( review =>
            getReviewInfo(review).then(foundReview => results.push(foundReview))))
        setReviews(results)
    }
    ...
    return (
        <div>
            ...
            {self && <div>
                ...
                <p><strong>Favorites: </strong> {userProfile.favorite}</p>
            </div>}
